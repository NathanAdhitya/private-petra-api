...
export class KHS {
    ...
    async getAllValidPeriods(): Promise<PeriodeKHS[]> {
        ...
        const combinations = await Promise.all(
            combinationResponses.map(async (result) => {
                ...
                return [
                    await this.parsePeriod(new JSDOM(textResult)).catch(
                        async (e) => {
                            console.error(e);
    ...
    */
    ...
    async parseCPMK(
        dom: JSDOM
    ): Promise<CapaianPembelajaranMataKuliah[] | null> {
        /*
      Table format is:
        ...
        const cplTable = [...table.values()].find(
            (table) =>
                table
                    .querySelector("tr")
                    ?.querySelector("td")
                    ?.textContent?.trim() === "Capaian Pembelajaran Lulus"
        );
    ...
    */
        ...
        for (const row of rows) {
            ...
            const persenKehadiran = Number.parseFloat(
                cells[5].textContent ?? ""
            );
            const detailId =
                cells[1]
